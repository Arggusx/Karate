        ...
        <div className="max-w-6xl mx-auto px-5 lg:px-8 flex flex-col gap-4">
          ...
          <div className="relative max-w-xl mx-auto w-full">
            ...
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-black/40 hover:text-jp-red"
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {FILTERS.map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2 tracking-widest text-xs uppercase transition-colors flex items-center gap-2 ${
                ...
                return (
                  <Reveal key={t.nome} delay={Math.min(i * 40, 320)}>
                    <button
                      ...
                      className="card-elev w-full text-left bg-white border border-black/5 overflow-hidden group"
                    >
      ...
      {tec && (
        ...
        >
          ...
          >
            <button
              onClick={() => setOpen(null)}
              className="absolute top-3 right-3 p-2 hover:bg-black/5 z-10 text-jp-ink"
              aria-label="Fechar"
            >
              <X size={20} />
            </button>
